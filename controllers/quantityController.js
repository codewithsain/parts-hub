    ...
    try {
        dbConn.query("SELECT id FROM part WHERE partNumber = ?", [partNumber], (error, results) =>{
            if(error){
                ...
                let id = results[0].id;
                try {
                    dbConn.query("SELECT id, globalEstimation, buyerEstimation, quotedEstimation, createDate FROM quantity  WHERE partID = ? ORDER BY createDate DESC", [id] ,(error, results) =>{
                        if(error){
                            return res.send(error)
                        }else if(results){
